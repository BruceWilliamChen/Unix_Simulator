...
public class ConcurrentCommandBuilder {
	...
	public static List<ConcurrentFilter> createFiltersFromCommand(String command){
		//initialize the list that will hold all of the filters
				...
				//first check if it is running background
				command = command.trim();
				//then check if the command is a background command
				if (ConcurrentCommandBuilder.ifNeedsBackground(command)) command = command.substring(0, command.length() - 1);
				command = " " + command + " ";
	...
	/*
	 * check if running on the background
	 */
	public static boolean ifNeedsBackground(String command) {
		//just check the last char to see if it is "&"
