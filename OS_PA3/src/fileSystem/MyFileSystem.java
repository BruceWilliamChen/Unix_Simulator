...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
        ...
        //these two will be used to contribute the check conditions
        int blockNum  = seekPtr / Disk.BLOCK_SIZE;
        int blockOff  = seekPtr % Disk.BLOCK_SIZE;
        
        if(blockNum > 9) {
            //two cases, could handle or too large, and indirect first
        	int totalBlockNum = IndirectBlock.COUNT + IndirectBlock.COUNT * IndirectBlock.COUNT +
        			            IndirectBlock.COUNT * IndirectBlock.COUNT * IndirectBlock.COUNT;
        	if (blockNum < 10 + totalBlockNum) {
        		//legal small
        		if (blockNum < (10 + IndirectBlock.COUNT)) return singleIndirect(blockNum, blockOff, inode, mode);
        		if (blockNum < (10 + IndirectBlock.COUNT + IndirectBlock.COUNT * IndirectBlock.COUNT)) {
        			//exponetiaility determines the degree
        			return doubleIndirect(blockNum, blockOff, inode, mode);
        		}
        		//otherwise return triple
        		return tripleIndirect(blockNum, blockOff, inode, mode);
        	} else {
        		//in this case the file is too large to process
        		System.out.println("File too large. Unsupported");
        		System.exit(1);
        	}
        }
 ...
 // get indirect block from indirect block, return false if disk is full
    private boolean tryGetting(Inode inode, int index, IndirectBlock block){
    	//check the value in the pointers in inode
    	if (inode.ptr[index] == 0) {
    		//then assign a new value to it, to see if it's still 0
    		inode.ptr[index] = freeMap.find();
    		if (inode.ptr[index] == 0) return false;
    		//then write to the disk
    		disk.write(inode.ptr[index], block);
    		freeMap.save();
    	} else disk.read(inode.ptr[index], block);
    	//then return true eventually
    	return true;
    }
    
    private DirectBlock findBlock(IndirectBlock block, int index, int addr, int blockOff, MODE mode){
    	boolean ifNew = block.ptr[index] == 0 ? true : false;
    	
    	if (block.ptr[index] == 0) {
    		//then check mode
    		if (mode == MODE.r) return DirectBlock.hole;
    		else {
    			block.ptr[index] = freeMap.find();
    			if (block.ptr[index] == 0) return null;
    		}
    	}
    	//then write
    	disk.write(addr, block);
    	return new DirectBlock(disk, block.ptr[index], blockOff, ifNew);
    }
    
    
    // single indirect
    private DirectBlock singleIndirect(int blockNum, int blockOff, Inode inode, MODE mode){
    	IndirectBlock block1 = new IndirectBlock();   
    	//then return based on checking if the block is legit or not
    	if(tryGetting(inode, 10, block1)) return findBlock(block1, blockNum - 10, inode.ptr[10], blockOff, mode);
    	return null;
    }
    
    private boolean tryGetting(IndirectBlock block1, int index1, int addr, IndirectBlock block2){
    	if (block1.ptr[index1] == 0) {
    		block1.ptr[index1] = freeMap.find();
    		if (block1.ptr[index1] == 0) return false;
    		//now write for the address
    		disk.write(addr, block1);
    		freeMap.save();
    	} else disk.read(block1.ptr[index1], block2);
    ...
    private DirectBlock doubleIndirect(int blockNum, int blockOff, Inode inode, MODE mode){
    	IndirectBlock block1 = new IndirectBlock();
    	IndirectBlock block2 = new IndirectBlock();
    	
    	//then use two variables to try to hold their indices
    	...
    	boolean ifBlock1Exists = tryGetting(inode, 11, block1);
    	boolean ifBlock2Exists = tryGetting(block1, index1, inode.ptr[11], block2);
    	
    	//only return if both of them are true
    	if (ifBlock1Exists && ifBlock2Exists) return findBlock(block2, index2, block1.ptr[index1], blockOff, mode);
    	return null;
    }
    ...
    private DirectBlock tripleIndirect(int blockNum, int blockOff, Inode inode, MODE mode){
    	IndirectBlock block1 = new IndirectBlock();
    	IndirectBlock block2 = new IndirectBlock();
    	IndirectBlock block3 = new IndirectBlock();
    	...
    	boolean ifBlock1Exists = tryGetting(inode, 12, block1);
    	boolean ifBlock2Exists = tryGetting(block1, index1, inode.ptr[12], block2);
    	boolean ifBlock3Exists = tryGetting(block2, index2, block1.ptr[index1], block3);
    	
    	//all of them have to be found to be able to return
    	if (ifBlock1Exists && ifBlock2Exists && ifBlock3Exists) return findBlock(block3, index3, block2.ptr[index2], blockOff, mode);
